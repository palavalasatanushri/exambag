...
const EXAMS = [
  { id: 'mid1',    label: 'Mid 1',    total: 40,  color: '#C026D3', lightBg: '#FDF4FF', emoji: '📘' },
  ...
  if (!selectedSemester) {
    return (
      <div className="card p-10 text-center">
        ...
        <p className="font-bold text-lg" style={{color:'#4A044E'}}>No semester selected</p>
        <p className="text-sm mt-2" style={{color:'#C026D3'}}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4 flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-aberration" style={{color:'#4A044E'}}>Improvements</h2>
          <p className="text-sm mt-0.5" style={{color:'#C026D3'}}>Select a subject to view exam reflections</p>
        </div>
        {localSemesters.length > 0 && (
          <select 
            className="app-input text-xs py-1.5 px-2 bg-background font-bold shadow-sm" 
            style={{color: '#4A044E', borderColor: 'rgba(192,38,211,0.3)', width: 'auto'}}
      ...
      {isLoading ? (
        ...
        <div className="card p-8 text-center">
          ...
          <p className="font-semibold" style={{color:'#4A044E'}}>No subjects yet</p>
          <p className="text-sm mt-1" style={{color:'#C026D3'}}>Add subjects in the Hub tab first.</p>
        </div>
      ) : (
            ...
            return (
              <div key={subject.id} className="card overflow-hidden">
                ...
                <button
                  ...
                  style={{background: isExpanded ? '#FDF4FF' : '#FFFFFF'}}
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center text-base font-bold" style={{background:'rgba(192,38,211,0.15)', color:'#C026D3'}}>
                      {subject.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="text-left">
                      <p className="font-bold" style={{color:'#4A044E'}}>{subject.name}</p>
                      <p className="text-xs" style={{color:'#C026D3'}}>Tap to view exams</p>
                    </div>
                  </div>
                  {isExpanded ? <ChevronUp size={18} style={{color:'#C026D3'}} /> : <ChevronDown size={18} style={{color:'#E879F9'}} />}
                </button>

                {/* Exam tabs */}
                {isExpanded && (
                  <div className="border-t" style={{borderColor:'rgba(192,38,211,0.15)'}}>
                        ...
                        <div className="p-4 space-y-4" style={{background:'#FAFEFE'}}>
                          ...
                          <div className="card-sm p-4 flex items-center gap-5">
                            <div className="relative w-16 h-16 shrink-0">
                              ...
                              <div className="absolute inset-0 flex flex-col items-center justify-center">
                                ...
                                <span className="text-[9px]" style={{color:'#E879F9'}}>{pct}%</span>
                              </div>
                            </div>
                          ...
                          <div className="card-sm p-4 space-y-3">
                            <div className="flex items-center justify-between">
                              <p className="font-bold text-sm flex items-center gap-2" style={{color:'#4A044E'}}>
                                <CheckCircle2 size={15} style={{color:'#C026D3'}} /> Tips & Tricks
                              </p>
                            ...
                            {examItems.filter(i => i.type === 'tip').length === 0
                              ? <p className="text-xs" style={{color:'#E879F9'}}>No tips yet.</p>
                              : examItems.filter(i => i.type === 'tip').map(tip => (
                                <div key={tip.id} className="flex items-start gap-2 group">
                                  <CheckCircle2 size={13} className="mt-0.5 shrink-0" style={{color:'#C026D3'}} />
                          ...
                          <div className="card-sm p-4 space-y-3">
                            ...
                            {examItems.filter(i => i.type === 'drawback').length === 0
                              ? <p className="text-xs" style={{color:'#E879F9'}}>No challenges noted.</p>
                          ...
                          <div className="card-sm p-4 space-y-3">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              {examEvidence.map(ev => (
                                <div key={ev.id} className="relative group aspect-square rounded-xl overflow-hidden" style={{background:'#FDF4FF', border:`1px solid ${cfg.color}30`}}>

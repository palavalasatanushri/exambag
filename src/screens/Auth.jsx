...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 animate-fade-in" style={{background:'#FAF5FF'}}>
      {/* Soft decorative bg circles */}
      <div className="fixed top-0 right-0 w-72 h-72 rounded-full pointer-events-none" style={{background:'rgba(192,38,211,0.08)', transform:'translate(30%,-30%)'}} />
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="text-center mb-8">
          <img src="/logo.png" alt="Exambag Logo" className="w-20 h-20 mx-auto mb-4 rounded-2xl shadow-md" />
          <h1 className="text-3xl font-black text-aberration" style={{color:'#4A044E'}}>Exambag</h1>
          <p className="text-sm mt-1" style={{color:'#C026D3'}}>Your academic study hub</p>
        </div>

        <div className="card p-6 space-y-5">
          {/* Sign in / Sign up tabs */}
          <div className="flex rounded-xl p-1 gap-1" style={{background:'#FDF4FF'}}>
            {['Sign In', 'Create Account'].map((t, i) => (
              <button key={t}
                onClick={() => { setIsLogin(i === 0); setMsg(null); }}
                className="flex-1 py-2 rounded-lg text-sm font-bold transition-all"
                style={isLogin === (i === 0)
                  ? {background:'#C026D3', color:'#FFFFFF', boxShadow:'0 2px 8px rgba(192,38,211,0.3)'}
          ...
          {msg && (
            <div className="p-3 rounded-xl text-xs font-medium" style={{
              background: msg.type === 'success' ? 'rgba(192,38,211,0.12)' : 'rgba(220,107,107,0.1)',
              color: msg.type === 'success' ? '#3D7A6A' : '#DC6B6B',
              border: `1px solid ${msg.type === 'success' ? 'rgba(192,38,211,0.25)' : 'rgba(220,107,107,0.2)'}`,
            }}>{msg.text}</div>
          )}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPass(!showPass)}
                  className="absolute right-3 top-1/2 -translate-y-1/2" style={{color:'#E879F9'}}>
          ...
          <p className="text-center text-xs" style={{color:'#E879F9'}}>
